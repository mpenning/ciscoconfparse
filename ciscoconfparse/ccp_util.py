    ...
    def __init__(self, v4addr_prefixlen=f"0.0.0.1/{IPV4_MAX_PREFIXLEN}", strict=False, debug=0): # nosec
        ...
        try:
            ...
            raise AddressValueError(
                "Could not parse '{}' (type: {}) into an IPv4 Address. {}".format(
                    v4addr_prefixlen, type(v4addr_prefixlen), str(eee)
                )
            )
        ...
        if isinstance(v4addr_prefixlen, str):
            ...
            if v4_str_rgx is not None:
                ...
                raise AddressValueError(
                    "Could not parse '{}' (type: {}) into an IPv4 Address".format(
                        v4addr_prefixlen, type(v4addr_prefixlen)
                    )
                )
    ...
    def parse_text_list(self, text, debug=False):
        ...
        for idx, _csv_part in enumerate(csv_parts):
            ...
            if idx == 0:
                ...
                if "-" not in _csv_part:
                    if debug is True:
                        logger.info(f"    idx: {idx} at point01, Appending {begin_obj}{os.linesep}")
                    expanded_interfaces.append(copy.deepcopy(begin_obj))
                    continue
