...
ppp_intfs = parse.find_objects_w_child(r"^interf", r"encapsulation ppp")
...
for obj in active_intfs:
    ...
    ip_addr = obj.re_match_iter_typed(r'ip\saddress\s(\S+)', default='N/A')
    ip_mask = obj.re_match_iter_typed(r'ip\saddress\s\S+\s+(\S+)', default='N/A')
    ...
    print("  IP mask: {0}".format(ip_mask))
